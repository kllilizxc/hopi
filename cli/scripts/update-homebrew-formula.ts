/**
 * Update the Homebrew formula for hopi.
 *
 * This script:
 * 1. Reads checksums from release-artifacts/checksums.txt
 * 2. Generates an updated hopi.rb formula
...
class Hopi < Formula
  ...
  homepage "https://github.com/tiann/hopi"
  version "${version}"
  license "MIT"

  on_macos do
    if Hardware::CPU.arm?
      url "https://github.com/tiann/hopi/releases/download/v#{version}/hopi-darwin-arm64.tar.gz"
      sha256 "${shas.darwinArm64}"
    else
      url "https://github.com/tiann/hopi/releases/download/v#{version}/hopi-darwin-x64.tar.gz"
  ...
  on_linux do
    if Hardware::CPU.arm?
      url "https://github.com/tiann/hopi/releases/download/v#{version}/hopi-linux-arm64.tar.gz"
      sha256 "${shas.linuxArm64}"
    else
      url "https://github.com/tiann/hopi/releases/download/v#{version}/hopi-linux-x64.tar.gz"
  ...
  test do
    assert_match version.to_s, shell_output("#{bin}/hopi --version")
  end
end
`;
}
...
async function main(): Promise<void> {
    ...
    if (!shouldPush) {
        ...
        const localFormulaPath = join(localFormulaDir, 'hopi.rb');
    ...
    try {
        ...
        // Write formula
        const formulaPath = join(formulaDir, 'hopi.rb');
        ...
        // Commit and push
        execSync('git add Formula/hopi.rb', { cwd: tempDir, stdio: 'pipe' });

        try {
            execSync(`git commit -m "Update hopi to v${version}"`, { cwd: tempDir, stdio: 'pipe' });
            execSync('git push origin main', { cwd: tempDir, stdio: 'pipe' });
            console.log(`\nSuccessfully pushed hopi v${version} to homebrew-tap`);
        } catch {
            console.log('\nNo changes to commit (formula already up to date)');
        }

        console.log('\nUsers can now install via:');
        console.log('  brew install tiann/tap/hopi');
    } finally {
        // Cleanup
