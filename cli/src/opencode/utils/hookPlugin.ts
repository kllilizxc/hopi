...
import { PRODUCT_ENV, PRODUCT_HEADERS, PRODUCT_NAME, PRODUCT_SLUG } from '@hopi/protocol/brand';

const PLUGIN_FILENAME = `${PRODUCT_SLUG}-hook.ts`;
...
function buildPluginSource(hookUrl: string, token: string): string {
    ...
    return [
        ...
        `const HOOK_URL = process.env.${PRODUCT_ENV.OPENCODE_HOOK_URL} || DEFAULT_HOOK_URL;`,
        `const HOOK_TOKEN = process.env.${PRODUCT_ENV.OPENCODE_HOOK_TOKEN} || DEFAULT_HOOK_TOKEN;`,
        '',
        ...
        `                '${PRODUCT_HEADERS.HOOK_TOKEN}': HOOK_TOKEN`,
        '            },',
        '            body',
        '        });',
        '    } catch {',
        ...
        '}',
        '',
        'export const HopiHookPlugin = async () => {',
        '    return {',
        '        event: async ({ event }) => {',
