...
async function runnerPost(path: string, body?: any): Promise<{ error?: string } | any> {
  ...
  try {
    const timeout = process.env[PRODUCT_ENV.RUNNER_HTTP_TIMEOUT] ? parseInt(process.env[PRODUCT_ENV.RUNNER_HTTP_TIMEOUT]!) : 10_000;
 ...
 * For instance we are not handling the case where we upgraded hopi,
 ...
 * For instance when running `hopi runner status` we can show more information.
 */
...
export async function isRunnerRunningCurrentlyInstalledHappyVersion(): Promise<boolean> {
  ...
  try {
    ...
    // we will get a new path or not when hopi is upgraded globally.
