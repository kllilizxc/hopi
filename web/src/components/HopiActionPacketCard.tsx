...
function actionToneClass(action: HopiActionView): string {
    if (action.status === 'blocked') {
        return 'app-shadow-control bg-red-500/5'
    }
    if (action.status === 'finished') {
        return 'app-shadow-control bg-emerald-500/5'
    }
    if (action.status === 'in_review') {
        return 'app-shadow-control bg-amber-500/5'
    }
    return 'app-shadow-control bg-[var(--app-subtle-bg)]'
}
    ...
    return (
        <div className={cn('rounded-lg p-3', actionToneClass(props.action))}>
            <div className="mb-3 flex min-w-0 flex-wrap items-center gap-2">
                ...
                <div className="rounded bg-[var(--app-secondary-bg)] px-1.5 py-0.5 font-mono text-[11px] text-[var(--app-hint)] shadow-sm">
                    {props.action.type}
                </div>
            </div>
...
export function HopiActionPacketCard(props: { packet: HopiActionPacketView }) {
    ...
    return (
        <div className="min-w-0 max-w-full rounded-lg app-shadow-surface bg-[var(--app-bg)] p-3">
