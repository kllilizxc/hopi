...
import type { PrototypeRemoteApiClient } from '@/prototype/remoteApi'
import {
    clearMessageWindow,
    fetchLatestMessages,
    fetchOlderMessages,
    ...
    type PrototypeSessionMessageState,
} from '@/lib/sessionMessageStore'

const EMPTY_STATE: PrototypeSessionMessageState = {
    sessionId: 'unknown',
    messages: [],
    pending: [],
    pendingCount: 0,
...
export function useSessionMessages(
    api: Pick<PrototypeRemoteApiClient, 'getMessages'> | null,
    sessionId: string | null,
): {
    messages: PrototypeSessionMessage[]
    warning: string | null
    isLoading: boolean
    ...
    const loadMore = useCallback(async () => {
        if (!api || !sessionId || !state.hasMore || state.isLoadingMore) {
            return
        }
