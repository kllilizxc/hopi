...
The hub automatically creates a "Hopi Voice Assistant" agent in your ElevenLabs account on first use.
...
```
Browser → WebRTC → ElevenLabs ConvAI → Voice Assistant → HOPI Hub → Coding Agent
```

The voice connection uses WebRTC for low-latency audio streaming. The HOPI hub provides conversation tokens and handles authentication.
