...
[Happy](https://github.com/slopus/happy) is an excellent project. So why build HOPI?

**The short answer**: Happy uses a centralized server that stores your encrypted data. HOPI is decentralized — each user runs their own hub, and the relay server only forwards encrypted traffic without storing anything. These different goals lead to fundamentally different architectures.
...
**Choose HOPI if**: You want data sovereignty, self-hosting, and minimal setup.
...
Each user runs their own hub. HOPI offers two modes of remote access:

- **Self-hosted** (own server / Cloudflare Tunnel / Tailscale) — You control the full network path, no E2EE needed
- **Public relay** (`hopi hub --relay`) — E2E encrypted via tunwg (WireGuard + TLS); the relay only forwards opaque packets
...
| Aspect | Happy | HOPI (self-hosted) | HOPI (relay) |
|--------|-------|-------------------|--------------|
